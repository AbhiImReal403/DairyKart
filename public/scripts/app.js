...
const filterSearch = term => {
    ...
    Array.from(product.children)
        .forEach((product) =>{
            ...
            if(product.textContent.toLowerCase().trim().replace(/ /g, "").includes(term.replace(/ /g, "").trim().toLowerCase())){
                // console.log(product)
    ...
    Array.from(product.children)
    .filter((product) => {
        !product.textContent.toLowerCase().trim().replace(/ /g, "").includes(term.replace(/ /g, "").trim().toLowerCase());
